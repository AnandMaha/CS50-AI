...
class MinesweeperAI():
        ...
        for s1 in self.knowledge:
            for s2 in self.knowledge:
                if s1.cells.issubset(s2.cells):
                    sNew = Sentence(s2.cells - s1.cells, s2.count - s1.count)
                    safes = sNew.known_safes()
                    mines = sNew.known_mines()
                    if safes:
                        for safe in safes:
                            self.mark_safe(safe)
                    if mines:
    ...
    def make_random_move(self):
        ...
        while max_moves > 0:
            rand_move = (random.randrange(self.height), random.randrange(self.width))
            if rand_move not in self.moves_made | self.mines:
                return rand_move
            max_moves -= 1
        return None